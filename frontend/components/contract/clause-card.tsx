    ...
    <article className="space-y-4">
      <header className="space-y-1.5">
        <div className="flex items-baseline justify-normal gap-3">
          ...
          {isPending && (
            <p className="text-body-sm text-muted-foreground/70 font-mono uppercase tracking-[0.08em]">
              Pending
            </p>
          )}
        </div>
        <h3 className="text-foreground font-display text-[22px] font-medium leading-[1.2] tracking-[-0.01em]">
          {typeLabel}
        </h3>
      </header>
          ...
          <div className="max-w-[95ch] pt-1">
            ...
            >
              <div className="flex items-start gap-3">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <p
                    className={`text-caption font-mono uppercase tracking-[0.08em] ${stripStyle.label}`}
                  >
                    {stripStyle.text}
                  </p>
                  <p className="text-foreground text-[15px] leading-[1.5] font-medium">
                    {headline}
                  </p>
                </div>
                <ChevronDown
                  className={`text-muted-foreground mt-1 size-4 shrink-0 transition-transform duration-200 ease-out-strong ${
                    open ? "rotate-180" : ""
                  }`}
                  strokeWidth={1.5}
                  aria-hidden
            ...
            >
              <div className="overflow-hidden">
                <div className="border-border/40 mt-5 ml-4 space-y-7 border-l pl-5 pt-1">
                  {restOfExplanation && (
  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        className="border-border bg-card hover:bg-foreground/4 hover:border-border/80 focus-visible:bg-foreground/4 text-foreground/90 hover:text-foreground inline-flex items-center gap-2 rounded-sm border px-3 py-2 font-mono text-[11px] uppercase tracking-[0.08em] transition-colors duration-150 focus-visible:outline-none"
      >
        Compare to original
        <ChevronDown
          className={`size-3.5 transition-transform duration-200 ease-out-strong ${
      ...
      {showOriginal && (
        <div className="border-border/40 mt-2 border-t border-dashed pt-4">
          <p className="text-caption text-muted-foreground mb-2 font-mono uppercase tracking-[0.08em]">
            Original
          </p>
